...
public class Main {
    public static void main(String[] args) {
        ...
        int minVal = agent[0].score;
        char minCode = agent[0].codename;

        for(int i = 0;i<5;i++) {
            if(minVal > agent[i].score) {
                minCode = agent[i].codename;
                minVal = agent[i].score;
            }
        }

        System.out.print(minCode+" " +minVal);
    }
}